 ...
 * 
 */
public abstract class FunctionViewOld<T extends BaseActivity> extends AbFunctionView<T>
	...
	public FunctionViewOld(T activity) {
		super(activity);
	}
	...
	private void setRootView() {
		boolean hasKey = ViewConfiguration.get(activity).hasPermanentMenuKey();
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			if (hasKey) {
				view = (ViewGroup) activity.getLayoutInflater().inflate(R.layout.common_root_immersion, null);
			} else {
				view = (ViewGroup) activity.getLayoutInflater().inflate(R.layout.common_root_immersion_key, null);
			}
		} else {
			if (hasKey) {
				view = (ViewGroup) activity.getLayoutInflater().inflate(R.layout.common_root, null);
			} else {
				view = (ViewGroup) activity.getLayoutInflater().inflate(R.layout.common_root_key, null);
			}
		}
	 ...
	 * 隐藏标题栏并且显示沉浸式空间<br/>
	 */
	public void hideTitleBarAndShowSpace() {
		view.findViewById(R.id.viewgroup_common_title).setVisibility(View.GONE);
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			view.findViewById(R.id.viewgroup_common_title_immersion_fullscreen).setVisibility(View.VISIBLE);
		}
	}
	...
	public void setTitleBarBackgroundResource(int resid) {
		((View) view.findViewById(R.id.title_view).getParent()).setBackgroundResource(resid);
	}

	public void setTitleBarBackgroundColor(int color) {
		((View) view.findViewById(R.id.title_view).getParent()).setBackgroundColor(color);
	}
	...
	public void setContentBarMove() {
		findViewById(R.id.viewgroup_common_content).setBackgroundResource(0);
		((ViewGroup) findViewById(R.id.common_root))
				.addView(activity.getLayoutInflater().inflate(R.layout.common_bg_move, null), 0);
	}
