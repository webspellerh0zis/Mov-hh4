...
public class ImageProcess {
	...
	private Bitmap makeToBitmap(Matrix dataR, Matrix dataG, Matrix dataB, int width, int height) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				pix[index] = 0xff000000 | (dataR.get(x, y) << 16) | (dataG.get(x, y) << 8) | dataB.get(x, y);
				index++;
			} // x
		} // y
	...
	private Matrix eachEqualize(Matrix temp, int width, int height) {
		...
		for (i = 0; i < width; i++) {
			// 每列
			for (j = 0; j < height; j++) {
				temp.set(i, j, bMap[temp.get(i, j)]);
			}
		}
		return temp;
	}
	...
	private Matrix eachRobertGradient(Matrix tempM, int width, int height) {
		...
		for (j = 0; j < height - 1; j++) {
			for (i = 0; i < width - 1; i++) {
				int temp = Math.abs(tempM.get(i, j) - tempM.get(i, j + 1))
						+ Math.abs(tempM.get(i + 1, j) - tempM.get(i, j + 1));
				tempM.set(i, j, temp);
			}
		}
		return tempM;
	...
	private Matrix eachSobelGradient(Matrix tempM, int width, int height) {
		...
		for (i = 1; i < width - 1; i++) {
			for (j = 1; j < height - 1; j++) {
				int temp1 = Math.abs((tempM.get(i + 1, j - 1) + 2 * tempM.get(i + 1, j) + tempM.get(i + 1, j + 1))
						- ((tempM.get(i - 1, j - 1) + 2 * tempM.get(i - 1, j) + tempM.get(i - 1, j - 1))));
				int temp2 = Math.abs((tempM.get(i - 1, j + 1) + 2 * tempM.get(i, j + 1) + tempM.get(i + 1, j + 1))
						- ((tempM.get(i - 1, j - 1) + 2 * tempM.get(i, j - 1) + tempM.get(i + 1, j - 1))));
	...
	private Matrix eachLaplaceGradient(Matrix tempM, int width, int height) {
		...
		for (i = 1; i < width - 1; i++) {
			for (j = 1; j < height - 1; j++) {
				int temp = Math.abs(5 * tempM.get(i, j) - tempM.get(i + 1, j) - tempM.get(i - 1, j)
						- tempM.get(i, j + 1) - tempM.get(i, j - 1));
				resultMatrix.set(i, j, temp);
			}
		}
		return resultMatrix;
	...
	private Matrix eachLaplaceGradientBlue(Matrix tempM, int width, int height) {
		...
		for (i = 1; i < width - 1; i++) {
			for (j = 1; j < height - 1; j++) {
				int temp = Math.abs(4 * tempM.get(i, j) - tempM.get(i + 1, j) - tempM.get(i - 1, j)
						- tempM.get(i, j + 1) - tempM.get(i, j - 1));
				resultMatrix.set(i, j, temp);
			}
		}
	 ...
	 * 
	 */
	public Bitmap laplaceGradient(Bitmap bmp) {
